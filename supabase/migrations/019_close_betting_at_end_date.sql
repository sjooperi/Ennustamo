-- Close betting when market end_date has passed (MLB: end_date = game start).
-- Recreates place_bet from 012 with end_date guard.
...
begin
  ...
  into v_yes, v_no, v_status, v_end, v_opts, v_pools
  from public.markets
  ...
  -- Betting closes at end_date (for MLB = scheduled first pitch)
...
comment on function public.place_bet(uuid, text, numeric) is
  'Place bet; rejects when status<>open or end_date <= now() (MLB closes at first pitch).';

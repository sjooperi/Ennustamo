...
export type MlbScriptName =
  | 'import-mlb-markets.mjs'
  | 'sync-mlb-odds.mjs'
  | 'resolve-mlb-markets.mjs'

/**
 * Run an MLB .mjs script as a child process.
 * Avoids Next/Vercel dynamic-import issues with scripts outside the bundle.
 */
export function runMlbScript(
  scriptName: MlbScriptName,
  args: string[] = [],
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill('SIGTERM')
      reject(new Error(`MLB script timeout: ${scriptName}`))
    }, timeoutMs)
